...
instance fullRA : RelationAlgebra (Set (X × X)) where
  ...
  rdist x y z := by
    ...
    · rintro ⟨b, hxy, hz⟩
      ...
      · right
        exact ⟨b, hy, hz⟩
    · rintro (⟨b, hx, hz⟩ | ⟨b, hy, hz⟩)
      · exact ⟨b, Or.inl hx, hz⟩
      · exact ⟨b, Or.inr hy, hz⟩
  comp_one x := by
    ext p
    rcases p with ⟨a, b⟩
    constructor
...
instance : RelationAlgebra (Set G) where
  ...
  assoc x y z := by
    ext g
    constructor
    · rintro ⟨xy, ⟨a, ha, b, hb, rfl⟩, c, hc, rfl⟩
      exact ⟨a, ha, b * c, ⟨b, hb, c, hc, rfl⟩, by rw [mul_assoc]⟩
    · rintro ⟨a, ha, yz, ⟨b, hb, c, hc, rfl⟩, rfl⟩
      exact ⟨a * b, ⟨a, ha, b, hb, rfl⟩, c, hc, by rw [mul_assoc]⟩
  rdist x y z := by
    ext g
    constructor
    · rintro ⟨a, hxy, b, hb, rfl⟩
      rcases hxy with hx | hy
      · left
        exact ⟨a, hx, b, hb, rfl⟩
      · right
        exact ⟨a, hy, b, hb, rfl⟩
    · rintro (⟨a, hx, b, hb, rfl⟩ | ⟨a, hy, b, hb, rfl⟩)
      · exact ⟨a, Or.inl hx, b, hb, rfl⟩
      · exact ⟨a, Or.inr hy, b, hb, rfl⟩
  comp_one x := by
    ext g
    constructor
    · rintro ⟨a, ha, b, hb, hg⟩
  ...
  conv_dist x y := by
    ...
    · rintro ⟨a, hxy, rfl⟩
      ...
      · right
        exact ⟨a, hy, rfl⟩
    · rintro (⟨a, hx, rfl⟩ | ⟨a, hy, rfl⟩)
      · exact ⟨a, Or.inl hx, rfl⟩
      · exact ⟨a, Or.inr hy, rfl⟩
  conv_comp x y := by
    ext g
    constructor
    · rintro ⟨a, ⟨b, hb, c, hc, rfl⟩, rfl⟩
      exact ⟨c⁻¹, ⟨c, hc, rfl⟩, b⁻¹, ⟨b, hb, rfl⟩, by simp [mul_inv_rev]⟩
    · rintro ⟨a, ⟨c, hc, rfl⟩, b, ⟨d, hd, rfl⟩, hg⟩
...
example : False → Z₃.e = Z₃.a := by trivial
example : ∃ x : Z₃, x = Z₃.e → Z₃.a = Z₃.a := by
  exists Z₃.e
  intro
  rfl

set_option diagnostics false
example : ∃ x : Z₃, Z₃.unary x := by exists Z₃.e
example : True → Z₃.e = Z₃.e := by
  intro
  rfl
example : Z₃.ternary Z₃.e Z₃.e Z₃.e := trivial
...
instance : AtomStructure (Z₃) where
  ...
  identity1 x y u := by
    cases x <;> cases y <;> cases u <;> simp [Z₃.unary, Z₃.ternary]
  identity2 x y := by
    cases x <;> cases y <;> exact ⟨Z₃.e, by intro; trivial⟩
  assoc u x y z w := by
    intro h
...
lemma assocr (_u x y z w : S) : R₃ y z v ∧ R₃ x v w → ∃ u : S, R₃ x y u ∧ R₃ u z w := by
...
theorem comp_assoc {a b : X} : (a, b) ∈ com (com R S) T → (a, b) ∈ com R (com S T) := by
  intro h
  rcases h with ⟨z, h₁, _⟩
  rcases h₁ with ⟨x,_,_⟩
  use x
...
example {a b : X} : (a,b) ∈ x ∧ (a,b) ∈ y → (a,b) ∈ x ∩ y := by
  intro h
  constructor
  rcases h with ⟨h1,h2⟩
  trivial
